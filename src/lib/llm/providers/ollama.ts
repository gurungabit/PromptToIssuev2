    ...
    try {
      ...
      try {
        ...
        if (modelsResponse.data?.models) {
          const availableModels = modelsResponse.data.models.map((model: { name: string }) => model.name);
    ...
    try {
      ...
      if (mode === "ticket") {
        ...
        return {
          ...
          suggestions: ["Try rephrasing your question", "Check if Ollama is running", "Verify the model is installed"]
        };
      }
    }
  }
